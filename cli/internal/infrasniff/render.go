...
func (r *Result) RenderNextDeployYAML(appName string) string {
	...
	fmt.Fprintf(&b, "app:\n  name: %s\n", appName)
	// Domain is configured here in the file (not prompted). Commented out so the
	// app deploys on the default *.workers.dev host until you set one. Use a bare
	// hostname, or the block form to record the registrar + how DNS is managed.
	b.WriteString("  # domain: app.example.com\n")
	b.WriteString("  # domain:\n")
	b.WriteString("  #   name: app.example.com\n")
	b.WriteString("  #   provider: cloudflare   # namecheap | cloudflare | other\n")
	b.WriteString("  #   dns: auto              # auto (provider API) | manual (print records)\n")
	...
	if block == "" {
		...
		// SuggestedCloudflareBlock already starts at "    cloudflare:".
		b.WriteString(block)
	}
	return b.String()
}
